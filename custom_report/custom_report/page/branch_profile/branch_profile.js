...
function render_branch_info(b) {
	if (!b) {
		$("#branch-info-card").html(`<div class="dashboard-card"><div class="empty-state">No Branch Found</div></div>`);
		return;
	}

	// Format established date if available
	const establishedDate = b.established_date || "15-Jan-2014"; // Placeholder
	const address = b.address || "Shri Ji Complex, Gayatri Mandir Road., opp. IndianOil, Gondia, Maharashtra 441614"; // Placeholder
...
function load_branch_profile(sol) {
	frappe.call({
		...
		args: { sol_id: sol },
		callback: (r) => {
			render_branch_manager(r.message);
			render_account_portfolio(r.message);
			render_branch_performance(r.message);
			render_staff_manpower(r.message);
		},
	});
}
...
function render_branch_manager(data) {
	if (!data) {
		$("#branch-manager-card").html(`<div class="dashboard-card"><div class="empty-state">No Manager Data</div></div>`);
		return;
	}

	// Get manager initials for avatar
	const managerName = data.bm_name || "CHETAN DAYARAM RATHOD";
	const initials = managerName.split(" ").map(n => n[0]).join("").substring(0, 2).toUpperCase();
	const experience = data.bm_vintage || "8 YEARS EXPERIENCE";
	const phone = data.bm_phone || "87953579248";
...
function render_account_portfolio(data) {
	if (!data) {
		$("#account-portfolio-summary").html(`<div class="empty-state">No Portfolio Data</div>`);
		return;
	}

	const totalAccounts = data.total_accounts || 6627;
	const totalYtdAchievement = data.total_ytd_achievement || 6627;
	const totalClosingBalance = data.total_closing_balance || 6627;
	const totalYtdTarget = data.total_ytd_target || 6627;
		...
		<div>
			...
			<div class="portfolio-grid">
				...
				<div class="portfolio-metric">
					...
					<div class="portfolio-metric-value">${formatNumber(totalYtdAchievement)}</div>
				</div>
				<div class="portfolio-metric">
					<div class="portfolio-metric-label">Total Closing Balance</div>
					<div class="portfolio-metric-value">${formatNumber(totalClosingBalance)}</div>
				</div>
				<div class="portfolio-metric">
					<div class="portfolio-metric-label">Total YTD Target</div>
					<div class="portfolio-metric-value">${formatNumber(totalYtdTarget)}</div>
				</div>
			</div>
		</div>
	`;
...
function render_branch_performance(data) {
	if (!data) {
		$("#branch-performance-card").html(`<div class="dashboard-card"><div class="empty-state">No Performance Data</div></div>`);
		return;
	}
	...
	const html = `
		<div class="dashboard-card" style="animation-delay: 0.15s;">
			...
			<div class="performance-item">
				...
				<div class="performance-values">${formatCurrency(monthAchieved)} / ${formatCurrency(monthTarget)}</div>
			...
			<div class="performance-item">
				...
				<div class="performance-values">${formatCurrency(ytdAchieved)} / ${formatCurrency(ytdTarget)}</div>
			...
			<div class="performance-item">
				...
				<div class="performance-values">${formatCurrency(yearlyAchieved)} / ${formatCurrency(yearlyTarget)}</div>
...
function render_staff_manpower(data) {
	if (!data) {
		$("#staff-manpower-card").html(`<div class="dashboard-card"><div class="empty-state">No Staff Data</div></div>`);
		return;
	}
